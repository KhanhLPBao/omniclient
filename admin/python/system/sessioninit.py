...
scriptdir = "D:/source/omniclient/admin/python/system"
serverIP = ''       #IP of server contain fastq files
...
session_source = f'{sessiondir}/{sessionname}'
with open(f'{session_source}.files') as listfile,\
    open(f'{session_source}.jobs') as listjobs:
    listfile = [_a for _a in listfile.readlines()]
    listjobs = [_b for _b in listjobs.readlines()]
###################################################################################################################
...
def job_lvl1():
    ...
    fileanalyzed = {}
    for job, file in [_c.split('|')[1] for _c in listfile]:
        ...
        match job:
            case 'ftp': #Import through import folder on workstation or any storage device that has ftp server enabled
                mkdirresults = subcommand(['mkdir',f'{serverstoragedir}/{ses}'])
                if mkdirresults == 0:
                    ...
                    while downloadresults is False:
                        downloadlink = [
                                'bash', f'{scriptdir}/ftp.sh',
                                serverIP,'guest','123456',
                                file,f'{req_progress}/input/L1/{sessionname}/{filename}',
                                ]
...
def job_lvl2and3(job:list,level):
    #   1: Create pendind
    for _task in job:
        writeresults = sub.getstatusoutput(
